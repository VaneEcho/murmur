...
用户可能在一次口述中讲了多天的内容。请：
1. 识别原文中提到的日期（如「6月8号」「昨天」「前天」「上周五」），把内容按所属日期拆分；没有明确属于其他日期的内容归入默认日期。同一天的内容只输出一个对象。
2. 对每一天的内容，按用户的整理要求处理文字。text 中不要包含日期或星期的抬头（程序会自动添加）。
3. 重要：text 内部每一件事独占一行，行与行之间用换行符分隔（JSON 字符串中写作 \\n）。绝不要把几件事挤成一段。只有一件事特别长时才在这件事内部换行。

只输出一个 JSON 数组，不要任何其他内容。格式示例（注意 text 里用 \\n 分隔每件事）：
[{{"date": "2026-06-03", "text": "上午开监测车去看了考场。\\n中午跟老李在平沙吃了麦当劳。\\n下午开会定了高考保障方案。"}}]
...
async def _chat(messages: list, url: str, api_key: str, model: str, think: bool = False) -> str:
    """优先走 Ollama 原生 /api/chat；非 Ollama 接口自动回落到 OpenAI 兼容端点。"""
        ...
        if _native_ollama.get(root, True):
            try:
                resp = await client.post(
                    f"{root}/api/chat",
                    json={"model": model, "think": think, "stream": False, "messages": messages},
                    headers=headers,
                )
...
async def merge_rewrite(
    ...
    raw = f"这一天已有的记录：\n{old}\n\n新补充的口述内容：\n{new}\n\n请将两者合并整理为一条完整的记录。"
    entries = await split_and_polish(raw, date, prompt_template, url, api_key, model, glossary, think=think)
...
- "entries": 仅 diary 时输出，其他类型为 []。一条记录可能一次补写了好几天的内容，请按天拆分成数组：[{{"date": "YYYY-MM-DD", "text": "..."}}, ...]
  ...
  - text：该天内容的轻度整理。只做三件事：改正语音输入的错字同音字、理顺明显不通的语句、每件事独占一行（行与行用换行符分隔，JSON 字符串中写作 \\n，如 "找到了站点掉线的原因。\\n手机换了套餐。"，绝不要挤成一段）。保留原本的口语说法，不要改成书面语或公文腔（例如「中午应该是有炸鸡腿」最多理顺成「中午吃了炸鸡腿」，绝不能写成「午餐摄入了炸鸡腿」）。不增加、不删减、不脑补。去掉开头的日期和星期（程序会另行添加）
...
async def classify_memo(
    ...
    reply = await _chat([{"role": "user", "content": prompt}], url, api_key, model, think=think)
    try:
        data = json.loads(_strip_fences(reply))
        t = data.get("type")
